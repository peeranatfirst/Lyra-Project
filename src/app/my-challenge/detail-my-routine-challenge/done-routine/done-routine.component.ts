...
@Component({
  selector: 'app-done-routine',
  templateUrl: './done-routine.component.html',
  styleUrls: ['./done-routine.component.css']
})
export class DoneRoutineComponent implements OnInit {
  ...
  onCheckSubmit(){
    ...
    firebase.database().ref('/users/' + this.uid + '/Challenges/' + this.chaId + '/scheduled/' + this.dayId).update({
      status: "checked",
      datetimeStamp: stamp
    });
    this.router.navigate(['/detailmyRoutinechallenge/'+this.chaId]);
